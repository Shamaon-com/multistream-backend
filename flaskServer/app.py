    ...
    def newStream(self):
        if not self.checkCurrent():
            ...
            app.logger.debug("ID: {broadcast_id}, \nKey: {key}".format(
                broadcast_id=self.live.broadcast_id, key=self.live.stream_key))
...
def multistream():
    ...
    # app.logger.debug(response['Items'])
    
    for item in response["Items"]:
        if not "streaming" in item:
            streaming = False
        else:
            streaming = item["streaming"]["BOOL"]
    
        if item["service"]['S'].lower() == appName and item["active"]['BOOL'] and not streaming:
            item["streaming"] = {'BOOL': True}
            client.put_item(
                TableName=tableName,
                Item=item
            )
            if appName == "instagram":
                ig = Instagram(item)
                app.logger.debug(
                    getAdress(item["service"]['S'].lower()) + ig.newStream())
                return redirect(getAdress(item["service"]['S'].lower()) + ig.newStream(), code=302)

            if appName == "facebook":
                return redirect(getAdress(item["service"]['S'].lower()) + item['sk']['S'] + item['fbKey']['S'], code=302)

            app.logger.debug(
                getAdress(item["service"]['S'].lower()) + item['sk']['S'])
...
def check():
    ...
    for item in response["Items"]:
        ...
        if "active" in item and item["active"]['BOOL'] and not streaming:
            return "Allowed", 200
    return "Denied", 403
